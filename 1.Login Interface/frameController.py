...
def leftFrame(default):
    ...
    loginButtonP1 = ttk.Button(frameP1, text="Iniciar Sesion", command=lambda: (logged(
        frameP1, nickNameP1Entry.get(), 1, default) if login(nickNameP1Entry.get(), passwordP1Entry.get(), default) == True else None))
...
def rightFrame(default):
    ...
    loginButtonP2 = ttk.Button(frameP2, text="Iniciar Sesion", command=lambda: (logged(
        frameP2, nickNameP2Entry.get(),2,default) if login(nickNameP2Entry.get(), passwordP2Entry.get(), default) == True else None))
...
def selectPfp(username, frame):
    ...
    if file:
        ...
        destination_path = os.path.join(destinationDir, fileName)
        shutil.copy2(file, destination_path)

        print(f"Imagen copiada a {destination_path}")

        updateFilePath(destination_path, username)
...
def remakeMidFrame():
    ...
    if user1IsLogged and user2IsLogged:
        sendButton.config(state="normal")  # Habilitar el botón si ambos usuarios están logueados
    else:
        sendButton.config(state="disabled")  # Deshabilitar el botón si al menos uno de los usuarios no está logueado
...
def logged(frame, name, frameNum, globalFrame):
    global user1IsLogged, user2IsLogged
    clean(frame)
